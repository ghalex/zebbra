...
<Dropdown
  ...
  trigger={(item, onTrigger) => (
    <Button variant="success" icon="cog" onClick={onTrigger} />
  )}
/>;
```
  ...
  trigger={(item, onTrigger) => (
    <Group>
      <Button
        ...
        static
      />
      <Button variant="primary" onClick={onTrigger}>
        {toLabels(state.selectedItems) || "Please select"}
      </Button>
    </Group>
  )}
>
        ...
        return (
          <Menu.Item key={i}>
            <Checkbox
              ...
              onChange={(evt, chk) => toggle(data, chk)}
            >
              {data.name}
            </Checkbox>
          </Menu.Item>
